...
class fm_rx(gr.top_block):
	def __init__(self, station):
		...
		self.sample_rate = 25e4

		self.audio_sink = audio.sink(self.sample_rate, "", True)

		self.usrp_source = osmosdr.source(args="numchan=1")
		self.usrp_source.set_sample_rate(self.sample_rate)
		...
		self.rational_resampler = filt.rational_resampler_ccc(interpolation=1, decimation=4, taps=None, fractional_bw=None)
		self.filter = filt.fir_filter_ccf(1, filt.firdes.low_pass(1, self.sample_rate, 1e5, 1e6, filt.firdes.WIN_HAMMING, 6.76))
		self.wbfm_rcv = analog.wfm_rcv(quad_rate=5e5, audio_decimation=10)


		self.connect(self.usrp_source, self.rational_resampler)
		self.connect(self.wbfm_rcv, self.audio_sink)

		## Connection setup number 1 (without filter)
